    ...
    , ui(new Ui::GZipWidget)
{
    ...
    connect(ui->openTextButton, &QPushButton::clicked, this, [&]{
        const QString path = QFileDialog::getOpenFileName(this, tr("Open file"), !openedInputFile.isEmpty() ? QFileInfo(openedInputFile).dir().absolutePath() : QDir::homePath(), "*.txt");
        if(!path.isEmpty())
        {
            QFile file(path);
    ...
    connect(ui->openGZipButton, &QPushButton::clicked, this, [&]{
        const QString path = QFileDialog::getOpenFileName(this, tr("Open file"), !openedOutputFile.isEmpty() ? QFileInfo(openedOutputFile).dir().absolutePath() : QDir::homePath(), "*.gz");
        if(!path.isEmpty())
        {
            QFile file(path);
            file.open(QIODevice::ReadOnly);
...
void GZipWidget::saveAs()
{
    ...
    if(option != TextEdits::none)
    {
        const QString path = QFileDialog::getSaveFileName(this, tr("Save As"), !openedInputFile.isEmpty()
                                                                                        && option == TextEdits::input ? QFileInfo(openedInputFile).dir().absolutePath() : !openedOutputFile.isEmpty()
                                                                                        && option == TextEdits::output ? QFileInfo(openedOutputFile).dir().absolutePath() : QDir::homePath(),
                                                                                        option == TextEdits::input ? "*.txt" : "*.gz");
        if(!path.isEmpty())
        {
            QFile file(path);
            if(file.open(QIODevice::WriteOnly))
...
void GZipWidget::open()
{
    ...
    if(option != TextEdits::none)
    {
        const QString path = QFileDialog::getOpenFileName(this, tr("Open"), !openedInputFile.isEmpty()
                                                                                    && option == TextEdits::input ? QFileInfo(openedInputFile).dir().absolutePath() : !openedOutputFile.isEmpty()
                                                                                    && option == TextEdits::output ? QFileInfo(openedOutputFile).dir().absolutePath() : QDir::homePath(),
                                                                                    option == TextEdits::input ? "*.txt" : "*.gz");
        if(!path.isEmpty())
